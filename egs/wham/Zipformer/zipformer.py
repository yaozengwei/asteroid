...
class Zipformer2(nn.Module):
    ...
    def __init__(
        ...
        if in_channels != encoder_dim[0]:
            self.in_layer = nn.Linear(in_channels, encoder_dim[0])
        else:
            self.in_layer = None
        self.in_norm = BiasNorm(encoder_dim[0], channel_dim=-1)
        ...
        self.first_out = nn.Sequential(nn.Linear(max(encoder_dim), n_src * in_channels),
                                       nn.PReLU())
        # Gating and masking in 2D space (after fold)
        self.net_out = nn.Sequential(nn.Linear(in_channels, in_channels), nn.Tanh())
        self.net_gate = nn.Sequential(nn.Linear(in_channels, in_channels), nn.Sigmoid())
    ...
    def forward(
        ...
        Args:
          x:
            The input tensor. Its shape is (batch_size, in_channels, seq_len).
          src_key_padding_mask:
            The mask for padding, of shape (batch_size, seq_len); True means
            masked position. May be None.
        Returns:
          Return a tuple containing 2 tensors:
            - embeddings: its shape is (batch_size, n_src, in_channels, seq_len)
        """
        ...
        x = self.first_out(x)  # (seq_len, batch_size, n_src*in_channels)
        x = x.reshape(seq_len, batch_size, self.n_src, in_channels)

        x = self.net_out(x) * self.net_gate(x)
        x = x.permute(1, 2, 3, 0)
        # now x: (batch_size, n_src, in_channels, seq_len)
...
class Model(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert n_filters == separator.in_channels, (n_filters, separator.in_channels)
        ...
        self.encoder_activation = SwooshR()

    def forward(self, x: Tensor) -> Tensor:
        """Enc/Mask/Dec model forward

        Args:
            wav (torch.Tensor): waveform tensor. 2D tensor, time last.
        ...
        tf_rep = self.encoder_activation(self.encoder(x))  # (batch, channels, seq_len)

        est_masks = self.separator(tf_rep)  # (batch, n_src, channels, seq_len)
        masked_tf_rep = est_masks * tf_rep.unsqueeze(1)  # (batch, n_src, channels, seq_len)

        decoded = self.decoder(masked_tf_rep)  # (batch, n_src, seq_len)
        reconstructed = nn.functional.pad(decoded, [0, time - decoded.shape[-1]])
...
def _test_zipformer_main():
    ...
    separator = Zipformer2(
        in_channels=n_filters,
        n_src=2,
        downsampling_factor=(1,2,4,8,4,2,1),
        encoder_dim=(64,128,192,256,192,128,64),
        encoder_unmasked_dim=(64,128,192,192,192,128,64),
        num_encoder_layers=(2,2,2,2,2,2,2),
        num_heads=(4,4,4,8,4,4,4),
        feedforward_dim=(192,384,512,768,512,384,192),
        cnn_module_kernel=(31,31,15,15,15,31,31),
    )
    model = Model(separator, n_filters=n_filters, n_src=2, kernel_size=16, stride=8)
    ...
    assert f.shape == (batch_size, 2, time), f.shape
